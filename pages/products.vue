...
export default {
  ...
  async asyncData(ctx) {
    ...
    let {
      status: status2,
      data: { areas, types }
    } = await ctx.$axios.get('/categroy/list', {
      params: {
        city
      }
    })
  ...
  methods: {
    appScroll: _.throttle(function() {
      let scrollTop =
        window.pageYOffset ||
        document.documentElement.scrollTop ||
        document.body.scrollTop
      let { offsetTop, offsetHeight } = this.$refs.lists.$children[0].$el

      if (scrollTop > offsetTop) {
        let index = Math.floor((scrollTop - offsetTop) / offsetHeight)
        this.point = this.allPoints[index]
      } else {
        this.point = this.allPoints[0]
      }
    }, 200)
  },
  destroyed() {
